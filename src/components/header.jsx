            ...
            <ul className="flex items-center gap-x-4 text-white text-xl">
                {navItems.map(({ path, label }) => (
                    <li key={label} className="py-2 px-4 font-bold">
                        <a href={path} className={`cursor-pointer ${isActive(path) ? 'text-[#195bff]' : ''}`}>
                            {label}
                        </a>
                    </li>
                ))}
                <li className="text-[#bacdff] font-medium flex cursor-pointer gap-1" onClick={togglePopOver}>
                    경영처 000님
                    <img src={HeaderArrow} alt="HeaderArrow" className={`${isOpen ? '' : 'scale-[-1]'}`} />
                </li>
                {isOpen && (
                    <div
